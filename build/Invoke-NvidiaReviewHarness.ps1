...
$projectEvidencePolicy = if ([string]::Equals($ReviewType, 'TEST_ARTIFACT', [StringComparison]::OrdinalIgnoreCase)) {
    ' PROJECT ACCEPTANCE POLICY (FACTUAL CONTEXT): The mandated hosted/local soak duration is exactly 10 minutes while migration CRs remain open. Do not report duration alone as a BLOCKER or HIGH finding. The upstream effective refresh cadence for RSS/AI is 30 minutes minimum/default; a faster scheduler poll is compatible only when cache freshness prevents extra external calls. Upstream optional AI summarization permits bounded retry/fallback behavior, so a trace containing a recovered HTTP 429 with eventual AI success is compatible with the upstream contract and is not evidence that first-attempt success is required. Existing, separately queued migration CRs, including reviewed YFinance baseline work, describe context outside this snapshot unless the current candidate introduces a new failure. Evaluate these facts against the supplied acceptance criteria and concrete current evidence; report only a confirmed current BLOCKER/HIGH defect.'
}
else {
    ''
}
