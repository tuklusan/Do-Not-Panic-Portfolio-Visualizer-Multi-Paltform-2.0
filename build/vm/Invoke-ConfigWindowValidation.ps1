...
function Invoke-WindowsValidation {
    ...
    if ($CaptureProductScene) {
        ...
        $scriptLines = @(
            ...
            '    }',
            '    if (-not [DnppvSceneNative]::MoveWindow($proc.MainWindowHandle, 24, 24, 1024, 768, $true)) {',
            '        throw ''MoveWindow rejected the required 1024x768 viewport.''',
            '    }',
            '    Start-Sleep -Seconds 2',
            ...
            '    if ($smallWidth -ne 1024 -or $smallHeight -ne 768) {',
            '        throw (''Required 1024x768 viewport was not achieved: {0}x{1}.'' -f $smallWidth, $smallHeight)',
            '    }',
            '    Add-Content -Path $stepPath -Value (''SCREEN={0},{1}'' -f $bounds.Width, $bounds.Height)',
            '    Add-Content -Path $stepPath -Value (''SMALL_VIEWPORT={0},{1}'' -f $smallWidth, $smallHeight)',
