...
function Invoke-LinuxValidation {
    ...
    $scriptLines = @(
        '#!/usr/bin/env bash',
        'set -euo pipefail',
        'DISPLAY_SOCKET=$(find /tmp/.X11-unix -maxdepth 1 -type s -name ''X*'' -print -quit)',
        'if [ -z "${DISPLAY_SOCKET:-}" ]; then echo "DISPLAY_NOT_FOUND" >> step.log; exit 1; fi',
        ...
        'echo "DISPLAY=$DISPLAY XAUTHORITY=$XAUTHORITY" >> step.log',
        ...
        ('DEADLINE=$((SECONDS + {0}))' -f $Timeout),
        'while (( SECONDS < DEADLINE )); do',
        '  WID=$(timeout --kill-after=2s 5 xdotool search --pid "$APPPID" | tail -n 1 || true)',
        '  if [ -n "${WID:-}" ]; then',
        '    break',
        '  fi',
        '  sleep 1',
    ...
    if ($CaptureProductScene) {
        ...
        $duplicateLines = if ($CaptureDuplicateInstanceFixture) {
            @(
                ...
                '  DUPWID=$(timeout --kill-after=2s 5 xdotool search --pid "$DUPPID" | tail -n 1 || true)',
        ...
        '  rm -f "$output"',
            ...
            "sleep $Warmup",
            ('DEFERRED_LANES_DEADLINE=$((SECONDS + {0}))' -f $Timeout),
            'while (( SECONDS < DEFERRED_LANES_DEADLINE )); do',
...
function Invoke-WindowsValidation {
    ...
    $scriptLines = @(
        ...
        '$existingProductProcesses = @(Get-Process -Name ''DoNotPanicPortfolioVisualizer.App'' -ErrorAction SilentlyContinue)',
        'if ($existingProductProcesses.Count -gt 0) { throw (''Pre-launch hard stop: existing DNPPV-2.0 process detected (PID(s): {0}). Close it and retry.'' -f (($existingProductProcesses | Select-Object -ExpandProperty Id) -join '', '')) }',
    ...
    if ($CaptureProductScene) {
        ...
        $scriptLines = @(
            ...
            '$startInfo.EnvironmentVariables[''DONOTPANICPORTFOLIOVISUALIZER2_LOCALDATA_ROOT''] = [string]$localDataRoot',
            ...
            '    $traceDeadline = (Get-Date).AddSeconds(90)',
            '    for ($attempt = 0; (Get-Date) -lt $traceDeadline; $attempt++) {',
            ...
            '        $traceInfo = Get-Item -LiteralPath $tracePath -ErrorAction SilentlyContinue',
            '        $traceTail = if ($traceInfo) { (Get-Content -LiteralPath $tracePath -Tail 3 -ErrorAction SilentlyContinue) -join '' || '' } else { ''TRACE_FILE_NOT_FOUND'' }',
            '        Add-Content -Path $stepPath -Value (''TRACE_WAIT attempt={0};length={1};last_write={2};tail={3}'' -f $attempt, $(if ($traceInfo) { $traceInfo.Length } else { 0 }), $(if ($traceInfo) { $traceInfo.LastWriteTimeUtc.ToString(''O'') } else { ''N/A'' }), $traceTail)',
            '        Start-Sleep -Milliseconds 500',
            '    }',
            '    if ([string]::IsNullOrWhiteSpace($windowedTrace)) {',
            '        $finalTraceTail = if (Test-Path -LiteralPath $tracePath -PathType Leaf) { (Get-Content -LiteralPath $tracePath -Tail 20 -ErrorAction SilentlyContinue) -join "`n" } else { ''TRACE_FILE_NOT_FOUND'' }',
            '        throw (''Timed out waiting for the documented Avalonia 1024x768 windowed-startup trace. TracePath={0}; Tail={1}'' -f $tracePath, $finalTraceTail)',
            '    }',
