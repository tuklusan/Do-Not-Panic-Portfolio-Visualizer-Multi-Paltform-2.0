...
function Invoke-WindowsValidation {
    ...
    if ($CaptureProductScene) {
        ...
        $scriptLines = @(
            ...
            '    Start-Sleep -Milliseconds 500',
            '    if ([DnppvSceneNative]::GetForegroundWindow() -ne $WindowHandle) {',
            '        throw (''Product window was not foreground before {0}.'' -f $State)',
            '    }',
            '}',
            ...
            '    $expandPattern = $fileMenu.GetCurrentPattern([System.Windows.Automation.ExpandCollapsePattern]::Pattern)',
            '    if ($null -eq $expandPattern) { throw ''The File menu item does not expose ExpandCollapsePattern.'' }',
            '    $expandPattern.Expand()',
            '    Start-Sleep -Milliseconds 500',
            '    if ($expandPattern.Current.ExpandCollapseState -ne [System.Windows.Automation.ExpandCollapseState]::Expanded) { throw ''The File menu item did not remain expanded for capture.'' }',
