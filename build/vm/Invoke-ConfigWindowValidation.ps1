...
function Invoke-LinuxValidation {
    ...
    if ($CaptureProductScene) {
        ...
        $scriptLines = @(
            ...
            "sleep $Warmup",
            'if ! timeout 15 bash -c ''until grep -q "STARTUP;SIGNAL=DEFERRED_LANES_STARTED" run.log; do sleep 1; done''; then echo "DEFERRED_LANES_NOT_STARTED" >> step.log; exit 1; fi',
