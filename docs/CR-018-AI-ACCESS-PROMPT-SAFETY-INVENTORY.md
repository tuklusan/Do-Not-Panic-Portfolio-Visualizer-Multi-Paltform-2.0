...
| AI-08 | Upstream settings/configuration workflow and physical validation path | Physically exercise the configuration and startup-probe path. Do not require a live provider or rendered AI summary: upstream covers provider responses with deterministic fake-HTTP tests, while the startup probe is non-blocking and failure permits normal retry. |
