...
| SOAK-03 | Retrieve both size-bounded circular log families: product and YFinance. | `build/Invoke-ProductSoak.ps1` and `trace/` artifact |
...
| SOAK-09 | In the active 10-minute soak mode, capture a timestamped settled real-product screenshot after warmup, canceling the capture loop during shutdown. | `DNPPV_PRODUCT_CAPTURE_INTERVAL_MINUTES=30`, timestamped PNG set, count/hash manifest, and artifact review |
...
matrix, four lab machines, NVIDIA NIM evidence review, and the locked
10-minute profile are tracked by CR-068 and CR-069.
